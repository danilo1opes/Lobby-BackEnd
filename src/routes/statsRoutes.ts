...
router.get('/stats', authMiddleware, async (req: Request, res: Response) => {
  try {
    ...
    const mediaAcessos =
      totalFotos > 0 ? Math.round(totalAcessos / totalFotos) : 0;

    // Foto mais acessada
    const fotoMaisAcessada = photos.reduce(
      (max, photo) => (photo.acessos > max.acessos ? photo : max),
      photos[0] || { acessos: 0, title: 'Nenhuma' },
    );
